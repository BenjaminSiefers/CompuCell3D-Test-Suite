...
Ex.
    ...
    python batch_test.init.py          
        """)
        exit(0)
    #creates the given number of instances to run of this program
    elif "-instances" in args:
        instances = args[args.index("-instances") + 1]
        del args[args.index("-instances") + 1]
        del args[args.index("-instances")]
        userGivenCommands = "";
        for arg in args:
    ...
    #created for multi-threading to give new threads the correct start time
        ...
        del args[args.index("__batchTestStartTime__")]
...
if not os.path.exists(testOutput + "SuccessfulResults" + timeStart + ".txt"):
    f = open(testOutput + "SuccessfulResults" + timeStart + ".txt", "w")
    f.write(platform.system() + " version: " + platform.version() + "  " + platform.machine() + "\n")
    if os.path.isfile("../../../CompuCell3D-64bit website.url"):
        f.write("CompuCell3D-64bit\n")
    elif os.path.isfile("../../../CompuCell3D-32bit website.url"):
